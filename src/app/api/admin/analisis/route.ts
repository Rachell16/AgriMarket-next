...
export async function GET() {
  try {
    ...
    const [
      ...
      supabaseAdmin.from('pesanan').select('created_at, total_bayar, status, metode_bayar').order('created_at', { ascending: true }),
      supabaseAdmin.from('detail_pesanan').select('produk_id, jumlah, subtotal, produk(nama)'),
      supabaseAdmin.from('produk').select('kategori_id, terjual, harga, stok, kategori(nama)'),
    ...
    pesanan?.forEach((p: any) => {
      if (p.status !== 'dibatalkan' && p.metode_bayar) {
        ...
        else if (m.includes('bri') || m.includes('transfer')) metodeBayar['Transfer BRI']++
    ...
    return NextResponse.json({
      ...
      produkTerlaris,
      distribusiKategori,
      summary: { totalPendapatan, totalOrder, orderSelesai, orderPending, totalKonsumen: totalKonsumen || 0, totalPetani: totalPetani || 0 }
    })
  } catch (e: any) {
